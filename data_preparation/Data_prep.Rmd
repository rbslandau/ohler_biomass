...
```{r, message=FALSE, warning=FALSE}
emergence <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/biomass_abundance_data/emergence_identification_92.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
#View(emergence)

emergence_NA_zero <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/biomass_abundance_data/emergence_NA_0_92.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
...
```{r, message=FALSE, warning=FALSE}
biomass_emergence <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/biomass_abundance_data/biomass_emergence_93.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = FALSE)

trap_days <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/biomass_abundance_data/no_trap_days.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
```
## Forest and vine
...
emergence_NA_zero <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/biomass_abundance_data/no_trap_days_NA.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
```
Adjust column names
```{r, message=FALSE, warning=FALSE}
...
Load data
```{r, message=FALSE, warning=FALSE}
generation_time <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/trait_data/generation_time_spear.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = TRUE)

Trait_EU <- readRDS(file = "/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/trait_data/Trait_DB_EU_corrected.rds")
```
## Size
...
#png(filename="diff_size_rat_class.png", width=15, height=10, units="in", res=300)
diff_size_rat
#dev.off()
```
...
#png(filename="diff_gen_rat.png", width=5, height=5, units="in", res=300)
diff_sen_rat
#dev.off()
```
...
## Load data
```{r, message=FALSE, warning=FALSE}
phys_chem <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/land_use_related_drivers_data/phys_chem.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
#View(phys_chem)
```
...
# Habitat parameter
```{r, message=FALSE, warning=FALSE}
habitat <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/land_use_related_drivers_data/habitat.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
```
## Correct mistakes in raw data and convert it into right form
Correct wrong date
```{r, message=FALSE, warning=FALSE}
...
```{r, message=FALSE, warning=FALSE}
psm_conc <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/pesticide_data/4_ANA_Rohdaten_MethodeVereinfacht_neu.txt", header = TRUE,sep = "\t", na.strings = c("NR", "NA"))
# View(psm_conc)
```
## Chose data from Rhineland Palatinate
```{r, message=FALSE, warning=FALSE}
psm_conc_RP <- filter(psm_conc, BL == "RP")
# View(psm_conc_RP)
```
## Add sample from April Dierbach forest (up)

* sample was labeled wrong, sometimes two values per substance
* lower concentration belongs to Dierbach up and higher concentration to Dierbach down
* values in 4_ANA_Rohdaten_MethodeVereinfacht_neu.txt are mean values of up and down site

```{r, message=FALSE, warning=FALSE}
dier_april <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/pesticide_data/Dierbach_April.csv", header = TRUE, sep = ",",na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
```
Dierbach-April-sample
```{r, message=FALSE, warning=FALSE}
dier_april_sample_ID <- unique(dier_april$samplingID)

psm_conc_RP_dier <- filter(psm_conc_RP, samplingID == dier_april_sample_ID)

psm_conc_RP_dier <- filter(psm_conc_RP_dier, conc != 0)
```
Table to compare values of 4_ANA and Dierbach-April-sample
```{r, message=FALSE, warning=FALSE}
compare <- dier_april 

compare$conc_psm_RP_dier <- with(compare, ifelse(subKgM == "2_4_D", 0.0189150, ifelse(subKgM == "boscalid",  0.0016635, ifelse(subKgM == "flufenacet_thiadone_AE_1258593", 0.0039610, ifelse(subKgM == "MCPA", 0.0018305, 0.0059075))))) 

names(compare)[14] <- c("conc_dier_april")

compare <- select(compare, c("subKgM", "CAS", "conc_dier_april", "conc_psm_RP_dier"))
#View(compare)
```
Table with concentration of Dierbach April samples
```{r, message=FALSE, warning=FALSE}
dier_april <- select(dier_april, c("BL", "siteID","samplingID", "sampleDate", "method", "subKgM", "subDeu", "CAS", "typeBVL", "conc", "LC50", "uba_rak"))

dier_april$siteID[dier_april$conc == 0.018160] <- "RP_DierR"
dier_april$siteID[dier_april$conc == 0.001362] <- "RP_DierR"
dier_april$siteID[dier_april$conc == 0.005742] <- "RP_DierR"

dier_april$samplingID <- with(dier_april, ifelse(siteID == "RP_Dier", dier_april_sample_ID, "RP_DierR_20180417"))
```
Remove "wrong"values of 4_ANA table (psm_conc_RP)
```{r, message=FALSE, warning=FALSE}
psm_conc_RP <- filter(psm_conc_RP, samplingID != dier_april_sample_ID)
```
Put variables into needed format to merge tables
```{r, message=FALSE, warning=FALSE}
psm_conc_RP$BL <- as.character(psm_conc_RP$BL)
psm_conc_RP$siteID <- as.character(psm_conc_RP$siteID)
psm_conc_RP$samplingID <- as.character(psm_conc_RP$samplingID)
psm_conc_RP$sampleDate <- as.character(psm_conc_RP$sampleDate)
psm_conc_RP$method <- as.character(psm_conc_RP$method)
psm_conc_RP$subKgM <- as.character(psm_conc_RP$subKgM)
psm_conc_RP$subDeu <- as.character(psm_conc_RP$subDeu)
psm_conc_RP$typeBVL <- as.character(psm_conc_RP$typeBVL)

dier_april$sampleDate <- gsub("20180417", "2018-04-17", dier_april$sampleDate)
```
Merge tables
```{r, message=FALSE, warning=FALSE}
psm_conc_RP <- bind_rows(psm_conc_RP, dier_april)
```
Put in needed format for calculations
```{r, message=FALSE, warning=FALSE}
psm_conc_RP$BL <- as.factor(psm_conc_RP$BL)
...
psm_conc_RP$typeBVL <- as.factor(psm_conc_RP$typeBVL)
```
## Add columns
Site column
```{r, message=FALSE, warning=FALSE}
...
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_KaiserR"] <- "Kaiserbach_forest"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Ise"] <- "Isenach_vine"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_IseR"] <- "Isenach_forest"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Krops"] <- "Kropsbach_vine"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_KropsR"] <- "Kropsbach_forest"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Trief"] <- "Triefenbach_vine"   
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_TriefR"] <- "Triefenbach_forest" 
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Moden"] <- "Modenbach_vine"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_ModenR"] <- "Modenbach_forest"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Moden"] <- "Modenbach_vine"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_ModenR"] <- "Modenbach_forest"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Otter"] <- "Otterbach_vine"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_OtterR"] <- "Otterbach_forest"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Kling"] <- "Klingbach_vine"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_KlingR"] <- "Klingbach_forest"
```
RP_HainR eds samples wrong name in table, actually this are samples from RP_Hain (down site; NOT up site)
```{r, message=FALSE, warning=FALSE}
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Hain"] <- "Hainbach_vine"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_HainR"] <- "Hainbach_forest"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Russ"] <- "Russbach_vine"        
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_RussR"] <- "Russbach_forest"   
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_Dier"] <- "Dierbach_vine"
psm_conc_RP$Site[psm_conc_RP$siteID == "RP_DierR"] <- "Dierbach_forest"

psm_conc_RP$Site[psm_conc_RP$Site == "Hainbach_forest" & psm_conc_RP$method == "eds"] <- "Hainbach_vine"
psm_conc_RP$Site[psm_conc_RP$Site == "Otterbach_forest" & psm_conc_RP$method == "eds"] <- "Otterbach_vine"
```
Add stream and position column
```{r, message=FALSE, warning=FALSE}
psm_conc_RP <- separate(psm_conc_RP, Site, c("stream", "position"), sep = "_", remove = FALSE)
```
Remove columns LC50 and uba_rak
```{r, message=FALSE, warning=FALSE}
psm_conc_RP <- select(psm_conc_RP,-c("LC50","uba_rak"))
```
...
```
## Remove metabolites + pharmaceuticals, to get only data for pesticides
```{r, message=FALSE, warning=FALSE}
unique(psm_conc_RP$typeBVL)
psm_conc_RP <- psm_conc_RP[!(psm_conc_RP$typeBVL == "P" | psm_conc_RP$typeBVL == "M"), ]
```
...
# View(unique(no_samples$no_samples))
```
...
psm_conc_RP$samplingID <- as.factor(psm_conc_RP$samplingID)
length(unique(psm_conc_RP$samplingID)) # now right
```
...
```{r, message=FALSE, warning=FALSE}
no_psm_type <- data.frame(cast(psm_conc_RP, samplingID  ~ typeBVL, value = "conc",  length))
#View(no_psm_type)
```
...
#### EDS
```{r, message=FALSE, warning=FALSE}
no_psm_type2 <- merge(no_psm_type, unique(psm_conc_RP[, c(3,5)]), by = c("samplingID"))
...
#### Schoepf
```{r, message=FALSE, warning=FALSE}
no_psm_type3 <- merge(no_psm_type, unique(psm_conc_RP[, c(3,5)]), by = c("samplingID"))
...
#### EDS and schoepf
```{r, message=FALSE, warning=FALSE}
det_compounds <- as.data.frame(unique(psm_conc_RP$subKgM)) # 67 pesticides
```
#### Schoepf samples
```{r, message=FALSE, warning=FALSE}
psm_conc_RP_grab <- filter(psm_conc_RP, method == "schoepf")
det_compounds_grab <- as.data.frame(unique(psm_conc_RP_grab$subKgM)) # 56 pesticides
```
#### EDS samples
```{r, message=FALSE, warning=FALSE}
...
You need to replace this line with a path on your computer
```{r, message=FALSE, warning=FALSE}
sed_conc <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/pesticide_data/20191111_KgM_SPM-Extrakte_2018_final_nur_RLP.csv", header = TRUE, sep = ",", na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
# View(sed_conc)

foc <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/pesticide_data/06_Schwebstoffilter GC_LC.csv", header = TRUE, sep = ",", na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
# View(foc)
```
...
Add typeBVL
```{r, message=FALSE, warning=FALSE}
psm_sed_conc$typeBVL <- with(psm_sed_conc, "I")
```
Add BL
```{r, message=FALSE, warning=FALSE}
psm_sed_conc$BL <- with(psm_sed_conc, "RP")
```
Add subDeu
```{r, message=FALSE, warning=FALSE}
...
```{r, message=FALSE, warning=FALSE}
psm_sed_conc$BL <- as.factor(psm_sed_conc$BL)
...
psm_sed_conc$typeBVL <- as.factor(psm_sed_conc$typeBVL)
...
if (run) {
  ...
  query_st = readRDS(file = "/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/pesticide_data/KGM_freshwater_invertebrate_XX50.rds")
}
...
```{r, message=FALSE, warning=FALSE}
EC50_manually <- read.table("/Users/katharinafrisch/Desktop/biomass_mit_unterordnern/pesticide_data/EC50_version_190326.csv", header = TRUE, sep = ",", na.strings = c("NR", "NA"), stringsAsFactors = FALSE)
# View(EC50_manually)
...
```{r, message=FALSE, warning=FALSE}
type_psm <- select(psm_conc_RP_pyr, c("CAS", "typeBVL"))
...
```{r, message =FALSE, warning= FALSE}
Date_season <- unique(psm_conc_RP_pyr[,c(14,15)])
...
#png(filename="max_TU_sumTU.png", width=7.5, height=7.5, units="in", res=300)
max_TU_sumTU
#dev.off()
```
...
#png(filename="diff_plot.png", width=20, height=10, units="in", res=300)
plot_diff
#dev.off()
```
...
#png(filename="diff_fam_ept.png", width=10, height=10, units="in", res=300)
plot_diff_fam_ept
#dev.off()
```
...
#write.csv(range_env_all, file = "range_env_all.csv", row.names = FALSE)
#write.csv(range_env_glmm, file = "range_env_glmm.csv", row.names = FALSE)
```

# Save data for statistics
## HGAM
```{r, message =FALSE, warning= FALSE}
#write.csv(biomass_emergence_sample, file = "biomass_emergence_sample.csv", row.names = FALSE)

#write.csv(biomass_emergence_order_sample, file = "biomass_emergence_order_sample.csv", row.names = FALSE)

#write.csv(biomass_emergence_family_sample, file = "biomass_emergence_family_sample.csv", row.names = FALSE)
```
## (G)LMM
```{r, message =FALSE, warning= FALSE}
#write.csv(data_env_1, file = "data_env_1.csv", row.names = FALSE)
```
## Paired t-test
```{r, message =FALSE, warning= FALSE}
#write.csv(trait_data, file = "trait_data.csv", row.names = FALSE)
#write.csv(trait_data_diff, file = "trait_data_diff.csv", row.names = FALSE)
```